...
the datset needed to be subset. There's a similar leftward skew in all regions,
...
between 120k - 240k copies when they've made it into the top 10k best selling 
...
Somewhat interesting (yet unspurprising) to see is that Electronic Arts, then 
...
strenghs we saw in the univariate plot for genre, though it's interesting to 
...
in franchsies like Halo and Fallout, so where it appears a game may not 
...
Nintendo. Following that, there aren't any surprizes seeing Electronic Arts 
...
With Nintendo and EA coming up again and again as a leaders in titles shipped, 
...
I wasn't sure how to compare these categorical varibles properly at first, so 
...
franchises on platforms. It's clear The Sims have predominently been PC games, 
...
1992, PC gaming bursted onto the scene with with a passionate fan base that 
...
in the 90's where Japan sold the most units world wide.
...
creating this plot showing each region as a porportion of the total gives a 
...
wars with either Xbox or PCs (which aren't all Microsft based anyways).